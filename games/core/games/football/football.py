...
class Config():
    ...
    IMAGE_PATHS_DICT = {
        ...
        'background_start': os.path.join(rootdir, 'resources/images/background_start.png'),
    }
    # 字体路径
    FONT_PATHS_DICT = {
        'default20': {'name': os.path.join(rootdir.replace('football', 'base'), 'resources/fonts/simkai.ttf'),
                      'size': 20},
        'default30': {'name': os.path.join(rootdir.replace('football', 'base'), 'resources/fonts/simkai.ttf'),
                      'size': 30},
        'default50': {'name': os.path.join(rootdir.replace('football', 'base'), 'resources/fonts/simkai.ttf'),
                      'size': 50},
    }
...
class FootballGame(PygameBaseGame):
    game_type = 'football'

    def __init__(self, **kwargs):
        self.cfg = Config
        super(FootballGame, self).__init__(config=self.cfg, **kwargs)
